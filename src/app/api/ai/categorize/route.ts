...
export async function POST(req: Request) {
  try {
    ...
    const categoriesSnap = await db
      .collection(`businesses/${body.businessId}/categories`)
      .get();

    const categories: Category[] = categoriesSnap.docs.map(
    ...
    const vendorsSnap = await db
      .collection(`businesses/${body.businessId}/vendors`)
      .get();

    const vendors: Vendor[] = vendorsSnap.docs.map(
      (d: QueryDocumentSnapshot) => ({
        id: d.id,
        name: d.data().name,
      })
    ...
    for (const chunkDrafts of chunks) {
      ...
      const reply = await jsonCompletion<{
        ...
        [
          {
            role: "system",
            content: `You categorize bank transactions.
Available categories (id|name|type):
${categories
  .map((c: Category) => `${c.id}|${c.name}|${c.type}`)
  .join("; ")}.

Available vendors (id|name):
${vendors
  .map((v: Vendor) => `${v.id}|${v.name}`)
  .join("; ")}`,
          },
          {
            role: "user",
